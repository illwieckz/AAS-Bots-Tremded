...
typedef struct botlib_import_s
{
	...
	//file system access
	int			(*FS_FOpenFile)( const char *qpath, fileHandle_t *file, enum FS_Mode mode );
	int			(*FS_Read)( void *buffer, int len, fileHandle_t f );
	int			(*FS_Write)( const void *buffer, int len, fileHandle_t f );
	void		(*FS_FCloseFile)( fileHandle_t f );
	int			(*FS_Seek)( fileHandle_t f, long offset, enum FS_Origin origin );//was int origin, lets try this
