...
void BotImport_Trace(bsp_trace_t *bsptrace, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int passent, int contentmask) {
	...
	SV_Trace(&trace, start, mins, maxs, end, passent, contentmask, TT_NONE);//Auriga: lets try this!
...
void BotImport_EntityTrace(bsp_trace_t *bsptrace, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int entnum, int contentmask) {
	...
	SV_ClipToEntity(&trace, start, mins, maxs, end, entnum, contentmask, TT_NONE);//Auriga: also here!
	//copy the trace information
	bsptrace->allsolid = trace.allsolid;
	bsptrace->startsolid = trace.startsolid;
	bsptrace->fraction = trace.fraction;
...
void SV_BotInitBotLib(void) {
	...
	if ( !Cvar_VariableValue("fs_restrict") && !Sys_CheckCD() ) {
		Com_Error( ERR_NEED_CD, "Game CD not in drive" ); //LOL
	}
	*/
