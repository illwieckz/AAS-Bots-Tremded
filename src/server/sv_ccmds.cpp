...
static void SV_Map_f( void ) {
	...
	// start up the map
	SV_SpawnServer( mapname, false ); //Auriga: modify later to kill bots or not
...
static void SV_MapRestart_f( void ) {
	...
	for (i=0 ; i<sv_maxclients->integer ; i++) {
		...
		denied = (char*)VM_ExplicitArgPtr( sv.gvm, VM_Call( sv.gvm, GAME_CLIENT_CONNECT, i, false, isBot ) );
		if ( denied ) {
