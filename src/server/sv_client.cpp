...
gotnewcl:	
	...
	*newcl = temp;
	...
	denied = VM_Call( sv.gvm, GAME_CLIENT_CONNECT, clientNum, true, false ); // firstTime = true
	if ( denied ) {
...
void SV_DropClient( client_t *drop, const char *reason ) {
	...
	if (drop->netchan.remoteAddress.type != NA_BOT) {//Auriga: dont do this for bots!!
		// see if we already have a challenge for this ip
		challenge = &svs.challenges[0];
	...
	// tell everyone  they got dropped
